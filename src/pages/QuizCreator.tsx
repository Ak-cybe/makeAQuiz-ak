...
import { Save, BookOpen, FileText, AlertCircle, CheckCircle2, Bot, Sparkles, ExternalLink, FileSpreadsheet } from "lucide-react";
import { QuizCategory, QuizQuestion } from "@/types/quiz";
import { toast } from "@/hooks/use-toast";
import { Alert, AlertDescription } from "@/components/ui/alert";
import { Badge } from "@/components/ui/badge";
import { CSVQuizUpload } from "@/components/quiz/CSVQuizUpload";
...
export function QuizCreator({ onSaveQuiz, onClose }: QuizCreatorProps) {
  ...
  const [csvQuestions, setCsvQuestions] = useState<QuizQuestion[]>([]);
  ...
  const handleSave = () => {
    ...
    if (activeQuestionCount === 0) {
      toast({
        title: "Error",
        description: "No questions found. Please upload a CSV or paste questions.",
        variant: "destructive",
      });
      return;
    }
    ...
    const newCategory: QuizCategory = {
      ...
      description: categoryDescription.trim() || `Custom quiz with ${activeQuestionCount} questions`,
      icon: "BookOpen",
    ...
    toast({
      title: "Quiz Created! 🎉",
      description: `"${categoryName}" with ${activeQuestionCount} questions has been added.`,
    });
  };

  const handleCSVQuestionsLoaded = (questions: QuizQuestion[]) => {
    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <div className="flex-1 pb-24">
        <div className="max-w-4xl mx-auto px-4 py-8">
          ...
          <div className="flex items-center justify-between mb-8">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-muted-foreground">Paste questions or upload a CSV file</p>
              </div>
            </div>
          ...
          {/* Tabbed Input: Paste Text / CSV Upload */}
          <Card className="mb-6">
            <CardContent className="p-0">
              <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                  ...
                  <TabsList className="w-full grid grid-cols-2 h-12 bg-muted/50">
                    ...
                    >
                      ...
                      {csvQuestions.length > 0 && (
                        <Badge variant="secondary" className="ml-1 h-5 px-1.5 text-[10px] bg-emerald-500/20 text-emerald-600">
                          {csvQuestions.length}
                        </Badge>
                      )}
                    </TabsTrigger>
                ...
                {/* CSV Upload Tab */}
                <TabsContent value="csv" className="p-6 mt-0">
                  <CSVQuizUpload onQuestionsLoaded={handleCSVQuestionsLoaded} />
                </TabsContent>
              </Tabs>
            </CardContent>
          </Card>

          {/* Example Format (only for paste tab) */}
          {activeTab === "paste" && (
            <Card className="mb-6">
              <CardHeader>
      ...
      <div className="sticky bottom-0 z-50 bg-background/95 backdrop-blur-md border-t border-border shadow-[0_-4px_20px_rgba(0,0,0,0.1)]">
          ...
          <div className="flex items-center gap-4">
            {activeQuestionCount > 0 && (
              <div className="hidden sm:flex items-center gap-2 text-sm">
                ...
                <span className="text-muted-foreground">
                  <strong className="text-foreground">{activeQuestionCount}</strong> questions ready
                </span>
              </div>
            )}
            <Button
              onClick={handleSave}
              size="lg"
              className="flex-1 gradient-primary text-white border-0 h-12 text-base font-bold shadow-lg hover:shadow-xl transition-shadow"
            ...
            >
              ...
              {activeQuestionCount === 0
                ...
                : `Save Quiz (${activeQuestionCount} Questions)`}
            </Button>
          </div>
        </div>
      </div>
